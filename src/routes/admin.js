...
// Rotas Get
router.get('/', (req,res) => {
    Catalogo.findAll().then((catalogo) => {
            res.locals.catalogo = catalogo.map(item => item.toJSON())
            res.render('admin/painel')
    }).catch((err) => {
        req.flash('error_msg', 'Erro ao buscar Catalogos')
        res.redirect('/')
    })
})

router.get('/edit/:nomeCatalogo', (req,res) => {
    const nomeCatalogo = req.params.nomeCatalogo

    Catalogo.findOne({where:{nomeCatalogo}}).then((catalogo) => {
        User.findOne({where:{id:catalogo.fk_id}}).then((usuario) => {
...
// Rotas Post
router.post('/edit/catalogo/:nomeCatalogo/:id', (req,res) => {
    let catalogo = req.params.nomeCatalogo
    ...
    if (erros.length >= 1) {
        ...
        return Promise.all([
            Catalogo.findOne({ where: { fk_id: userId } }),
            User.findOne({ where: { id: userId } })
        ]).then(([catalogo,usuario]) => {
            if (!catalogo) {
                req.flash('error_msg', 'Não foi possível mostrar os erros');
                return res.redirect(`/admin/edit/${catalogo}`);
            }
            res.locals.usuario = usuario.toJSON()
            res.locals.catalogo = catalogo.toJSON();
            return res.render(`admin/edit`, { erros });
        }).catch((err) => {
            console.error(err);
            req.flash('error_msg', 'Erro interno ao carregar catálogo e usúario');
            return res.redirect(`/admin/edit/${catalogo}`);
        });
    }

    // Verifica e atualiza
    Catalogo.findOne({ where: { fk_id: userId } }).then((catalogoUser) => {
        if (!catalogoUser) {
        ...
        if (catalogoUser.nomeCatalogo !== nomeCatalogo) {
            Catalogo.findOne({
                where: {
                    nomeCatalogo,
                    fk_id: { [Op.ne]: userId } // diferente do atual user
                }
                ...
                return catalogoUser.update({
                    ...
                    return res.redirect(`/admin/edit/${nomeCatalogo}`);
                }).catch((err) => {
                    console.error(err);
...
router.post('/edit/usuario/:id', (req, res) => {
    function capitalize(str) {
        return str.charAt(0).toUpperCase() + str.slice(1).toLowerCase();
    }

    const id = req.params.id;
    const nome = capitalize(req.body.nome.trim());
    const sobrenome = capitalize(req.body.sobrenome.trim());
    ...
    if (!sobrenome || sobrenome.length < 2) {
        erros.push({ text: 'Sobrenome inválido ou muito curto' });
    }
    if (!email) {
    ...
    if (erros.length >= 1) {
        // Recarrega view com os erros e os dados atuais
        return Promise.all([
            Catalogo.findOne({ where: { fk_id: id } }),
            User.findOne({ where: { id: id } })
        ]).then(([catalogo,usuario]) => {
            if (!catalogo) {
                req.flash('error_msg', 'Não foi possível mostrar os erros');
                return res.redirect(`/admin/edit/${catalogo.nomeCatalogo}`);
            }
            res.locals.usuario = usuario.toJSON()
            res.locals.catalogo = catalogo.toJSON();
            return res.render(`admin/edit`, { erros });
    ...
    return Promise.all([
            Catalogo.findOne({ where: { fk_id: id } }),
            User.findOne({ where: { id: id } })
        ]).then(([catalogo,usuarioAtual]) => {
            if (!usuarioAtual) {
                req.flash('error_msg', 'Usuário não encontrado');
                return res.redirect(`/admin/edit/${catalogo.nomeCatalogo}`);
            }

            // Se o e-mail mudou, verificar se já está em uso por outro
            if (usuarioAtual.email !== email) {
                return User.findOne({ where: { email } }).then((usuarioComMesmoEmail) => {
                    if (usuarioComMesmoEmail) {
                        req.flash('error_msg', 'E-mail já está em uso por outro usuário');
                        return res.redirect(`/admin/edit/${catalogo.nomeCatalogo}`);
                    }

                    // Atualiza com o novo e-mail
                    return usuarioAtual.update({
                        nome,
                        sobrenome,
                        email
                    }).then(() => {
...
router.post('/edit/senha/:id', (req, res) => {
    const id = req.params.id;
    ...
    if (senha.length < 8) {
        erros.push({ text: 'Senha muito curta (mínimo 8 caracteres)' });
    }
    if (senha !== senha2) {
        erros.push({ text: 'Senhas não coincidem' });
    }

    // Se houver erros, recarrega com os dados
    if (erros.length >= 1) {
        return Promise.all([
            Catalogo.findOne({ where: { fk_id: id } }),
            ...
            console.error('Erro ao carregar catálogo/usuário:', err.message);
            req.flash('error_msg', 'Erro interno ao carregar dados');
            return res.redirect('/admin/catalogos');
        });
    }
    ...
    .then(([catalogo, usuario]) => {
        if (!catalogo || !usuario) {
            req.flash('error_msg', 'Usuário ou catálogo não encontrado');
            return res.redirect(`/admin/edit/${catalogo.nomeCatalogo}`);
        }
        ...
        return bcrypt.genSalt(10)
            ...
            .then((senhaHash) => {
                return usuario.update({ senha: senhaHash });
            })
            .then(() => {
    ...
    .catch((err) => {
        ...
        req.flash('error_msg', 'Erro interno ao atualizar senha');
        return res.redirect(`/admin/edit/${catalogo.nomeCatalogo}`);
    });
});
